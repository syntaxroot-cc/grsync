...
// comparison against the destination's current state) still runs
// exactly as it would for a real transfer - see the individual
// receive* helpers below for the specific guarded calls, all eight of
// them audited: two os.MkdirAll calls and an os.WriteFile in
...
func Receiver(rw io.ReadWriter, dest string, attrOpts sync.AttrOptions, ropts ReceiverOptions) error {
	...
	for _, entry := range entries {
		...
		switch {
		case entry.IsDir:
			if err := receiveDir(destPath, entry, attrOpts, ropts); err != nil {
				return err
			}
		...
		case entry.Mode&fs.ModeSymlink != 0:
			if err := receiveSymlink(destPath, entry, attrOpts, ropts); err != nil {
				return err
			}
			continue
		...
		if err := receiveRegularFile(rw, destPath, entry, attrOpts, ropts); err != nil {
			return err
		}
	}
...
func receiveDir(destPath string, entry sync.FileEntry, attrOpts sync.AttrOptions, ropts ReceiverOptions) error {
	...
	code, report := itemizeDir(entry, old, existed, attrOpts)
	reportChange(ropts, code, report, entry)
	return nil
}
...
// comment), so there is nothing to write and nothing to report either -
// this short-circuit keeps that consistent rather than reporting a
// change that sync.ApplyAttributes would never actually have made.
func receiveSymlink(destPath string, entry sync.FileEntry, attrOpts sync.AttrOptions, ropts ReceiverOptions) error {
	if !attrOpts.Links {
		return nil
	}
	...
	code, report := itemizeSymlink(entry, old, existed, attrOpts)
	reportChange(ropts, code, report, entry)
	return nil
}
...
// work needed to report accurate itemize output; only the final
// os.WriteFile/os.MkdirAll/sync.ApplyAttributes calls are skipped.
func receiveRegularFile(rw io.ReadWriter, destPath string, entry sync.FileEntry, attrOpts sync.AttrOptions, ropts ReceiverOptions) error {
	old, existed, err := lstatExisting(destPath)
	if err != nil {
		return fmt.Errorf("checking existing %q: %w", entry.Path, err)
	}
	...
	code, report := itemizeFile(entry, old, existed, contentChanged, attrOpts)
	reportChange(ropts, code, report, entry)
	return nil
}
